...
async function registrarCelulasPadrao(){await window.PWAO_RegistrarCelula({nome:'home',caminho:'./celulas/home/index.html'});await window.PWAO_RegistrarCelula({nome:'finance',caminho:'./celulas/financeiro/index.html'});await window.PWAO_RegistrarCelula({nome:'education',caminho:'./celulas/educacao/index.html'});}

async function startOrganism({seedFresh=false}={}){await DataOrchestrator.init({seedIfFresh:seedFresh});await Memory.open();await registrarCelulasPadrao();navigate('home',{pushHistory:false});notify(seedFresh?'Dados iniciais prontos. Boas vindas!':'Bem-vindo de volta 👋',3000);}
