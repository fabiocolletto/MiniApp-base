const translations = {
    ...
    const initialLanguage =
      typeof documentLanguage === 'string' && translations[documentLanguage]
        ? documentLanguage
        : DEFAULT_LANGUAGE;
    ...
    const catalogButton = document.querySelector('[data-js="open-catalog"]');
    const catalogLabel = catalogButton
      ? catalogButton.querySelector('[data-js="catalog-label"]')
      : null;
    ...
    window.addEventListener('message', (event) => {
      ...
      if (state.lastGeneratedValues && !state.manualOverride) {
        const regeneratedScript = composeScript(state.lastGeneratedValues, locale);
    ...
    const synchronizeWithShell = () => {
      notifyShell();
      requestLocaleSync();
    };

    if (document.readyState === 'interactive' || document.readyState === 'complete') {
      synchronizeWithShell();
    } else {
      document.addEventListener('DOMContentLoaded', synchronizeWithShell);
    }
