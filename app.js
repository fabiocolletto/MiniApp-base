...
const PORTFOLIO_ITEMS = [
    ...
    { id: 'tasks', title: "Tarefas", view: 'tasks-menu', color: "amber", icon: "calendar_month" }, 
];

const EDUCATION_CATEGORIES = [
    { id: 'student', title: "Alunos", view: 'student-menu', color: "sky", icon: "person" },
    { id: 'responsible', title: "Responsáveis", view: 'responsible-menu', color: "violet", icon: "diversity_3", status: "Em breve" }, 
    { id: 'tutors', title: "Tutores", view: 'tutor-menu', color: "amber", icon: "supervisor_account", status: "Em breve" }, 
    { id: 'institution', title: "Instituições", view: 'institution-menu', color: "purple", icon: "apartment", status: "Em breve" }, 
];

const STUDENT_MENU_ITEMS = [
    { title: "Cadastro", action: 'modal', modalId: 'registration', url: 'https://cdn.jsdelivr.net/gh/fabiocolletto/miniapp@main/products/educacao/student-registration.html', basePath: 'https://cdn.jsdelivr.net/gh/fabiocolletto/miniapp@main/products/educacao/', color: "blue", icon: "app_registration" },
    { title: "Simulados", action: 'modal', modalId: 'simulados', url: 'https://cdn.jsdelivr.net/gh/fabiocolletto/miniapp@main/products/educacao/simulados.html', basePath: 'https://cdn.jsdelivr.net/gh/fabiocolletto/miniapp@main/products/educacao/', color: "pink", icon: "edit_note", status: "Em breve" },
    { title: "Agenda", action: 'placeholder', color: "teal", icon: "event", status: "Em breve" },
];

const TASKS_MENU_ITEMS = [
    { title: "Planejamento Semanal", action: 'modal', modalId: 'weekly-planner', url: 'https://cdn.jsdelivr.net/gh/fabiocolletto/miniapp@main/products/tarefas/weekly-planner.html', basePath: 'https://cdn.jsdelivr.net/gh/fabiocolletto/miniapp@main/products/tarefas/', color: "amber", icon: "event_note" }, 
    { title: "Lista de Tarefas", action: 'modal', modalId: 'tasks', url: 'https://cdn.jsdelivr.net/gh/fabiocolletto/miniapp@main/products/tarefas/tasks.html', basePath: 'https://cdn.jsdelivr.net/gh/fabiocolletto/miniapp@main/products/tarefas/', color: "red", icon: "check_box" }, 
    { title: "Agenda (Mensal)", action: 'modal', modalId: 'monthly-calendar', url: 'https://cdn.jsdelivr.net/gh/fabiocolletto/miniapp@main/products/tarefas/monthly-calendar.html', basePath: 'https://cdn.jsdelivr.net/gh/fabiocolletto/miniapp@main/products/tarefas/', color: "blue", icon: "calendar_month" }, 
...
const VIEW_MAP = {
    ...
    'tasks-menu': { title: "Tarefas e Produtividade", subtitle: "Gerencie seus compromissos e foco.", data: TASKS_MENU_ITEMS, parent: 'portfolio' },
...
const AppCard = ({ item, onClick }) => {
    const opacity = item.status === "Em breve" ? 'opacity-50 pointer-events-none' : '';
    const clickHandler = item.status === "Em breve" ? (e) => e.preventDefault() : onClick;
    ...
    return (
        ...
        >
            ...
            <div className="relative z-10 flex flex-col justify-end h-full text-center pb-2">
                <h2 className={`text-sm text-${item.color}-700 font-semibold leading-tight`}>{item.title}</h2>
            </div>
        </div>
    );
};
...
const MiniAppModal = ({ modalConfig, closeModal, loading, setLoading, appId, fbConfig }) => {
    ...
    const loadMiniAppContent = useCallback(async () => {
        ...
        try {
            ...
            iframeRef.current.srcdoc = `<div style="text-align:center; padding:2rem; font-family:sans-serif;">
                <h3 style="color:#ef4444; margin:0;">Erro de Carregamento</h3>
                <p style="color:#9ca3af; font-size:0.9rem;">O arquivo <code>${url.split('/').pop()}</code> falhou ao carregar.</p>
            </div>`;
            setLoading(false);
        }
        ...
        <div id={`${modalId}-modal-overlay`} tabIndex="-1" aria-hidden="true" 
            ...
            <div className="relative w-full max-w-5xl h-full bg-white rounded-2xl shadow-2xl border border-indigo-500 flex flex-col overflow-hidden animate-fade-in-up">
                ...
                <iframe 
                    ...
                    title={`App ${modalConfig.title || modalId}`}
                ></iframe>
                ...
                {loading && (
                    <div className="absolute inset-0 z-30 flex justify-center items-center bg-white/80 backdrop-blur-sm">
                        ...
                        <p className="ml-3 text-sm font-semibold text-gray-700">Carregando MiniApp...</p>
                    </div>
                )}
            </div>
        </div>
    ...
    const AlertPanel = () => {
        return (
            <div className="p-6 text-center">
                ...
                <p className="text-gray-600 mt-4">O sistema de notificações via Planilha Google está ativo. Novas notificações aparecerão aqui após o login.</p>
                <p className="mt-2 text-sm text-gray-500">Usuário atual: {userPhone || 'Não Logado'}</p>
            ...
            <div className="fixed top-4 left-4 z-40 flex flex-col space-y-2">
                {history.length > 0 && (
                    <button onClick={goBack} 
                        ...
                        <span className={textClasses}>Voltar</span>
                    </button>
                )}
                <button onClick={() => navigateTo('portfolio', false)} 
                    ...
                    <span className={`hidden sm:${textClasses}`}>Início</span> 
                </button>
                <button onClick={() => handleCardClick({ action: 'modal', modalId: 'alerts', title: 'Painel de Alertas', url: 'ALERT_PANEL_PLACEHOLDER', basePath: '' })} 
                    ...
                    <span className={`hidden sm:${textClasses}`}>Alertas</span>
                </button>
            </div>
            ...
            <div className="fixed top-4 right-4 z-40">
                <button onClick={openAuthModal} 
                    ...
                    <span className={`hidden sm:${isNavMinimized ? 'hidden' : 'inline'}`}>
                        {userPhone ? userPhone : 'Entrar/Cadastrar'}
                    </span>
            ...
            {placeholderVisible && (
                <div id="placeholder-modal-overlay" className="fixed inset-0 z-50 flex justify-center items-center w-full h-full bg-black/60 backdrop-blur-sm p-4">
                    <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl border border-gray-300 flex flex-col overflow-hidden p-8 text-center">
                        ...
                        <h3 className="text-xl font-bold text-gray-800">Recurso em Desenvolvimento</h3>
                        <p className="text-gray-600 mt-2">Esta funcionalidade estará disponível em breve. Agradecemos sua paciência!</p>
